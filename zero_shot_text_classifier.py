...
from dashboard_utils.gui import keyboard_to_url, load_keyboard_class
# Imports for aggrid
...
if selected == "Demo (5 phrases max)":
    ...
    if not submit_button and not st.session_state.valid_inputs_received:
        ...
        st.warning(
            "❄️ Please make sure to add at least two labels for classification")
        st.session_state.valid_inputs_received = False
        st.stop()
    ...
    if not submit_button and not st.session_state.valid_inputs_received:
        ...
        try:
            ...
            st.warning(
                "❄️ Add a valid HuggingFace API key in the text box above ☝️")
            st.stop()
